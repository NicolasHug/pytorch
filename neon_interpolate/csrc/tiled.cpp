...
void scalar_vertical_conv_uint8(
    uint8_t* out,
    ...
    unsigned int precision) {
  const int32_t round = 1 << (precision - 1);
  const int64_t row_stride = data_size;
  const uint8_t* base = buffer + ids_min;

  for (int64_t j = 0; j < data_size; j++) {
    int32_t acc = round;
    for (int64_t i = 0; i < ids_size; i++) {
      acc += static_cast<int32_t>(base[i * row_stride + j]) * weights[i];
    }
...
void tiled_bilinear_uint8(const at::Tensor& input, const at::Tensor& output) {
  ...
  // Tiling: pick tile_h (output rows per tile) so the strip buffer stays in L2.
  // Strip buffer holds the horizontally-convolved input rows needed for one tile
  // of vertical convolution. For output rows [oy0, oy_end), we need approximately
  // tile_h * scale_y + kh input rows, each of size W_out * C bytes.
  //
  // Target: 75% of typical L2 (1MB) = 750KB.
  // Non-tiled intermediate would be H_in * W_out * C bytes (e.g. 6MB for 4K->1K),
  // which spills to L3/DRAM. The tiled version trades ~kh/(tile_h*scale_y) extra
  // horizontal compute (typically 3-5%) for the vertical pass reading from L2.
  ...
  // fprintf(stderr,
  //     "[tiled] %ldx%ld -> %ldx%ld | tile_h=%ld | input_tile_h=%ld | "
  //     "strip=%.1fKB (target=%.1fKB) | kh=%d | num_tiles=%ld\n",
  //     H_in, W_in, H_out, W_out, tile_h, max_input_tile_h,
  //     strip_bytes / 1024.0, target_bytes / 1024.0, vert_w.kmax,
  ...
  for (int64_t oy0 = 0; oy0 < H_out; oy0 += tile_h) {
    ...
    // Input row range for this strip (vert_w.xmin stores byte offsets)
...
at::Tensor upsample_bilinear2d_aa_tiled(
  ...
  tiled_bilinear_uint8(input_cl, output);
...
TORCH_LIBRARY(tiled_interpolate, m) {
  m.def("upsample_bilinear2d_aa(Tensor input, int[] output_size) -> Tensor");
}

TORCH_LIBRARY_IMPL(tiled_interpolate, CPU, m) {
  m.impl("upsample_bilinear2d_aa", &upsample_bilinear2d_aa_tiled);
}

PYBIND11_MODULE(_C_tiled, m) {
