...
struct InterpolateNonSeparable {
    static inline opmath_t eval(char* src, char** data, const int64_t* strides, int64_t i) {
      ...
      opmath_t t = InterpolateNonSeparable<n - 1, scalar_t, opmath_t, interp_size>::eval(src + ids, &data[2 * interp_size], &strides[2 * interp_size], i);
      ...
      for (const auto j : c10::irange(1, interp_size)) {
        ...
        t = InterpolateNonSeparable<n - 1, scalar_t, opmath_t, interp_size>::eval(src + ids, &data[2 * interp_size], &strides[2 * interp_size], i);
        output += t * wts;
      }
      return output;
  }
...
struct InterpolateNonSeparable<1, scalar_t, opmath_t, interp_size> {
    static inline opmath_t eval(char* src, char** data, const int64_t* strides, int64_t i) {
      int64_t ids = *(int64_t*)&data[0][i * strides[0]];
      opmath_t wts = *(scalar_t*)&data[1][i * strides[1]];
...
struct InterpolateNonSeparable<n, scalar_t, opmath_t, 1> {
    static inline opmath_t eval(char* src, char** data, const int64_t* strides, int64_t i) {
      int64_t ids = *(int64_t*)&data[0][i * strides[0]];
      return InterpolateNonSeparable<n - 1, scalar_t, opmath_t, 1>::eval(src + ids, &data[2], &strides[2], i);
  }
};

template <typename scalar_t, typename opmath_t>
struct InterpolateNonSeparable<1, scalar_t, opmath_t, 1> {
    static inline opmath_t eval(char* src, char** data, const int64_t* strides, int64_t i) {
      int64_t ids = *(int64_t*)&data[0][i * strides[0]];
...
// struct InterpolateNonSeparable<n, scalar_t, opmath_t, 2> and
// struct InterpolateNonSeparable<1, scalar_t, opmath_t, 2>
template <int n, typename scalar_t, typename opmath_t>
struct InterpolateNonSeparable<n, scalar_t, opmath_t, 2> {
    static inline opmath_t eval(char* src, char** data, const int64_t* strides, int64_t i) {
        ...
        opmath_t t0 = InterpolateNonSeparable<n - 1, scalar_t, opmath_t, 2>::eval(src + i0, &data[4], &strides[4], i);
        opmath_t t1 = InterpolateNonSeparable<n - 1, scalar_t, opmath_t, 2>::eval(src + i1, &data[4], &strides[4], i);
...
inline scalar_t interpolate_non_separable(char* src, char** data, const int64_t* strides, int64_t i) {
  ...
  return InterpolateNonSeparable<n, scalar_t, opmath_t, interp_size>::eval(src, data, strides, i);
}
...
inline void basic_loop_non_separable(char** data, const int64_t* strides, int64_t n) {
  ...
  for (const auto i : c10::irange(n)) {
    *(scalar_t*)&dst[i * strides[0]] = interpolate_non_separable<out_ndims, scalar_t, interp_size>(
...
inline void basic_loop_separable_1d_vertical(
  ...
  for (const auto i : c10::irange(n)) {
    *(scalar_t*)&dst[i * strides[0]] =
        interpolate_separable_1d_zero_strides<scalar_t>(
...
inline void basic_loop_separable_1d_horizontal(
  ...
  if (strides[1] == 0) {
    for (const auto i : c10::irange(n)) {
      *(scalar_t*)&dst[i * strides[0]] =
          interpolate_separable_1d<scalar_t>(
...
void upsample_non_separable(at::TensorIterator& iter)
{
  auto loop = [&](char** data, const int64_t* strides, int64_t n) {
    ...
    if ((strides[0] == sizeof(scalar_t) && (strides[1] == 0) &&
      ...
      basic_loop_non_separable<scalar_t, out_ndims, interp_size>(data, strides, n);
