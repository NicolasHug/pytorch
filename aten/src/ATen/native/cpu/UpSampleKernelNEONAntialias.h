...
// Horizontal interpolation for a single row (raw pointer interface).
// Used by the ring buffer path to resample individual rows on demand.
...
// Same NEON strategy as NeonResampleVertical, but reads from an array of
// row pointers instead of a contiguous tensor.
...
void upsample_neon_bilinear_bicubic_uint8(const at::Tensor& input_,
  ...
  if (need_horizontal && need_vertical) {
    // Ring buffer: fuse H+V passes. Keep only ksize_vert horizontally-resampled
    // rows in a small buffer that fits in L1 cache.
